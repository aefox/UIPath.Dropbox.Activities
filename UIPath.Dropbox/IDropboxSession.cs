...
namespace UIPath.Dropbox
{
    ...
    public interface IDropboxSession : IDisposable
    {
        ...
        Task DownloadFileAsync(string path, string downloadFolder, CancellationToken cancellationToken);

        Task DownloadFolderAsZipAsync(string path, string downloadFolder, string zipName, CancellationToken cancellationToken);
    }
}